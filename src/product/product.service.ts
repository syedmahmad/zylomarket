...
export class ProductService {
  ...
  async create(createProductDto: any, ownerId: number) {
    ...
    const merchantInfo = await this.merchantRepository.findOne({
      where: { userId: ownerId, storeId: store.id },
    });
    if (!merchantInfo) {
      throw new NotFoundException(
        `Merchant with owner ID ${ownerId} not found.`,
      );
    }
  ...
  async findAll(
    domain: string,
  ): Promise<{ products: Product[]; salesInfo: SalesCampaign | null }> {
    ...
    const salesInfo = await this.salesCampaignRepository.findOne({
      where: { storeId: store.id },
    });

    return { products, salesInfo };
  }
  ...
  async update(
    ...
    if (images || imagesToRemove) {
      ...
      if (images && images.length > 0) {
        ...
        const newImages = images.filter(
          (img) => !existingUrls.includes(img.imageUrl),
        );
    ...
    if (isSaleOn !== undefined) {
      if (isSaleOn && salesCampaignId) {
        const salesCampaign =
          await this.salesCampaignRepository.findByPk(salesCampaignId);
        if (!salesCampaign) {
          throw new NotFoundException(
            `Sales campaign with ID ${salesCampaignId} not found`,
          );
        }

        const existingCampaignProduct =
          await this.campaignProductRepository.findOne({
            where: {
